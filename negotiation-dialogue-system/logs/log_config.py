...
def setup_logging():
    ...
    dialogue_log.addHandler(logging.FileHandler("negotiation-dialogue-system/logs/dialogue_logs.txt", mode='a'))
    
    # 感情ログ
    emotion_log = logging.getLogger('emotion_log')
    emotion_log.setLevel(logging.INFO)
    emotion_log.addHandler(logging.FileHandler("negotiation-dialogue-system/logs/emotion_logs.txt", mode='a'))
    
    # 性格ログ
    personality_log = logging.getLogger('personality_log')
    personality_log.setLevel(logging.INFO)
    personality_log.addHandler(logging.FileHandler("negotiation-dialogue-system/logs/personality_logs.txt", mode='a'))
    
    # 意図ログ
    intent_log = logging.getLogger('intent_log')
    intent_log.setLevel(logging.INFO)
    intent_log.addHandler(logging.FileHandler("negotiation-dialogue-system/logs/intent_logs.txt", mode='a'))
    
    # リスクリターン値ログ
    risk_profit_log = logging.getLogger('risk_profit_log')
    risk_profit_log.setLevel(logging.INFO)
    risk_profit_log.addHandler(logging.FileHandler("negotiation-dialogue-system/logs/risk_profit_logs.txt", mode='a'))
    
    # 価格ログ
    price_log = logging.getLogger('price_log')
    price_log.setLevel(logging.INFO)
    price_log.addHandler(logging.FileHandler("negotiation-dialogue-system/logs/price_logs.txt", mode='a'))
    
    # 対話段階ログ
    phase_log = logging.getLogger('phase_log')
    phase_log.setLevel(logging.INFO)
    phase_log.addHandler(logging.FileHandler("negotiation-dialogue-system/logs/phase_logs.txt", mode='a'))